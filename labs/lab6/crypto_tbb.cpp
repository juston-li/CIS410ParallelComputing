...
struct xor_op{
  int keys;
  char cchar;
  xor_op(int numKeys, char cipherChar) : keys(numKeys),cchar(cipherChar) {}
  void operator() (const blocked_range<char>& r) {
    for(int keyLoop=0;keyLoop<keys;keyLoop++){
      cchar=cchar ^ getBit(&(keyList[keyLoop]),charLoop);
    }
  }
}
...
void encode(char* plainText, char* cypherText, xorKey* keyList, int ptextlen, int numKeys) {
  ...
  for(charLoop=0;charLoop<ptextlen;charLoop++) {
    xor_op(numKey,plainText[charloop]) cipherChar;
    parallel_reduce(blocked_range<char*>(plainText,plainText+numKeys),cipherChar);  
