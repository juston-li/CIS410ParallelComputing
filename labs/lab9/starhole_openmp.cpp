...
int walker(long int seed, int x, int y, int stepsremaining) {
    ...
    for( ; stepsremaining>0 ; stepsremaining-=2 ) {
        int spwn0 = 0;
        int spwn1 = 0;

	#pragma omp task shared(spwn0)
        ...
        if(doesSplit(&seedbuf, splitProb, x, y, radius)) {
            ...
            spwn0 = walker(seed + newseed, x, y, stepsremaining-1);
        }
        ...
        if(doesSplit(&seedbuf, splitProb, x, y, radius)) {
            ...
            long int newseed;
            lrand48_r(&seedbuf, &newseed);
            spwn1 = walker(seed + newseed, x, y, stepsremaining-2);
        }
	...
	#pragma omp taskwait
        particles = particles + spwn0 +spwn1;
    }
